...
<BlogLayout title={frontmatter.title}>
  ...
  When I started building this website with [astro](https://astro.build), I quickly searched for ways to optimize the website. What I wanted was static HTML with as little network requests as reasonably possible.
  ...
  Now, I wanted to optimize this somewhat, specifically reduce the network requests. Bundling the styles into the main HTML should be pretty easy. Astro has a built-in option for that: `inlineStylesheets`.
  ```ts
  ...
  Instead, we can make the browser load the style an scripts in the `head`, and only consider the render done when they are loaded.
